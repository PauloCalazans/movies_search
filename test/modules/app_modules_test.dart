...
main() {
  ...
  test('Deve recuperar o usecaseMovie sem erro', () {
    final usecaseMovie = Modular.get<SearchByTitle>();
    expect(usecaseMovie, isA<SearchByTitleImpl>());
  });

  test('Deve recuperar o usecaseMovieDetail sem erro', () {
    final usecaseMovieDetail = Modular.get<SearchByImdbID>();
    expect(usecaseMovieDetail, isA<SearchByImdbIDImpl>());
  });

  test('Deve recuperar uma lista de Movie', () async {
    when(dio.get(any)).thenAnswer((_) async => Response(data: jsonDecode(responseMoviesForTest), statusCode: 200));

    final usecaseMovie = Modular.get<SearchByTitle>();

    final result = await usecaseMovie("avengers", 1);
    expect(result | null, isA<List<Movie>>());
  });

  test('Deve recuperar um MovieDetail', () async {
